typedef struct hogar {
    ...
    char ciudad[TAMANIO_CIUDAD];
    int cantidad_anios;
} hogar_t;

void leer_hogar(FILE* fhogares, hogar_t* hogar, bool* es_eof) {
   int cantidad_leidos = fscanf(fhogares, "%[^;];%[^;];%[^;];%[^;];%[^;];%i\n",
      hogar->periodo, hogar->continente, hogar->pais, hogar->ciudad,
      &(hogar->cantidad_anios));
      ...
      while (!eof_fhogares && (strcmp(periodo_actual, hogar_leido.periodo) == 0)) {
         ...
         while (!eof_fhogares && (strcmp(periodo_actual, hogar_leido.periodo) == 0) &&
            (strcmp(continente_actual, hogar_leido.continente) == 0)) {
               ...
               printf("\t\tPaís: %s\n", hogar_leido.pais);
               int cantidad_anios_por_pais = 0;
               char pais_actual[TAMANIO_PAIS];
               strcpy(pais_actual, hogar_leido.pais);
               while (!eof_fhogares && (strcmp(periodo_actual, hogar_leido.periodo) == 0) &&
                  (strcmp(continente_actual, hogar_leido.continente) == 0) &&
                  (strcmp(pais_actual, hogar_leido.pais) == 0)) {

                  printf("\t\tCiudad: %s - Cantidad de años vividos acá: %i\n",
                     hogar_leido.ciudad, hogar_leido.cantidad_anios);
                  cantidad_anios_por_pais += hogar_leido.cantidad;
                  leer_hogar(fhogares, &hogar_leido, &eof_fhogares);
               }
               printf("\t\tCantidad de años vividos en el país %s: %i\n", pais_actual,
                  cantidad_anios_por_pais);
               cantidad_anios_por_continente += cantidad_anios_por_pais;
         }
